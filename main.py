...
def on_button_click():
    ...
    if len(faces) == 0:
        ...
        no_face_start_time = time.time()
        cv2.putText(template, text="Multiple Faces Detected!!!", org=(368, 50), lineType=cv2.LINE_AA, fontScale=0.8, color=(255, 255, 255), thickness=2, fontFace=cv2.FONT_HERSHEY_SIMPLEX)
        status = " Retry: 'r' & Quit: 'q'"
        return
...
while True:
    ...
    if no_face_start_time:
        ...
        if elapsed_time >= 1:
            cv2.rectangle(template, (368, 0), (694, 93), (204, 71, 63), -1)
            no_face_start_time = None
            status = " Retry: 'r' & Quit: 'q'"
